...
/** Map of UUID -> label for resolving sorry references. */
 ...
 * - `#s-<uuid>` -> link to /c/<uuid> with resolved label (sorry)
 * - `#p-<uuid>` -> link to /p/<uuid> (project)
 * - `#<uuid>` (bare hash) -> link to /c/<uuid> with resolved label
 * - Bare UUID (not in a link) -> link to /c/<uuid> with resolved label
...
function autoLink(text: string, refs?: ReferenceMap): string {
  ...
  // #s-<uuid> -> sorry link with resolved label
  result = result.replace(
    new RegExp(`#s-(${UUID_RE})`, 'g'),
    (_, id) => `[${resolveLabel(id, refs)}](/c/${id})`,
  )

  // #c-<uuid> -> sorry link (backward compat)
  result = result.replace(
